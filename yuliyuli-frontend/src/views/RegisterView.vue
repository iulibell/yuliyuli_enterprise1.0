          ...
          <el-form-item label="验证码" prop="code">
            <el-row :gutter="10">
              <el-col :span="16">
                ...
                style="margin-right: -40px; width: 140px;"/>
              </el-col>
              <el-col :span="8">
                <el-button 
                  type="primary" 
...
const sendCode = async () => {
  ...
  try {
    console.log('正在发送验证码，手机号：', registerForm.phone)
    ...
    ElMessage.error('发送验证码失败，请查看控制台')
  }
}
...
const handleRegister = async () => {
  if (registerFormRef.value) {
    await registerFormRef.value.validate(async (valid: boolean) => {
      if (valid) {
        try {
          const res = await userApi.register(registerForm, registerForm.code)
          if (res.success) {
