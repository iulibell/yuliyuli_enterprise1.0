...
public interface VideoMapper extends BaseMapper<Video> {
   ...
   */
  @Insert(
      "INSERT INTO video (`user_id`,`title`,`intro`,`url`,`cover`,`type_id`)"
   ...
   */
  @Update(
      "UPDATE video SET collection_count = collection_count + #{collectionCount} WHERE url = #{url}")
   ...
   * @return 影响行数
   */
  @Update("UPDATE video SET is_deleted = 1 WHERE url = #{url} and user_id = #{userId}")
